...
def main():
    ...
    required_files = [
        ...
        "docs/plans/2026-06-26-unique-location-notification-identifiers.md",
    ...
    require('content.body = "A new location was saved."' in app_delegate and
            ...
            failures)
    notification_handler = app_delegate[
        app_delegate.find("func newVisitReceived"):
        app_delegate.find("func locationManager(_ manager: CLLocationManager, didUpdateLocations", app_delegate.find("func newVisitReceived"))
    ]
    notification_identifier = 'let notificationIdentifier = "\\(location.date.timeIntervalSince1970)-\\(UUID().uuidString)"'
    identifier_index = notification_handler.find(notification_identifier)
    request_index = notification_handler.find("UNNotificationRequest(identifier: notificationIdentifier")
    require(0 <= identifier_index < request_index and
            "UNNotificationRequest(identifier: location.dateString" not in notification_handler,
            "local visit notifications must use unique timestamp-UUID request identifiers",
            failures)
    ...
    require("New location writes use timestamp-prefixed unique JSON filenames" in readme and
            ...
            failures)
    notification_identifier_plan = read("docs/plans/2026-06-26-unique-location-notification-identifiers.md")
    notification_identifier_verification = markdown_section(
        notification_identifier_plan, "Verification Completed"
    )
    normalized_notification_plan = " ".join(notification_identifier_plan.split())
    normalized_notification_verification = " ".join(notification_identifier_verification.split())
    require("status: completed" in notification_identifier_plan and
            "Apple documents that a reused notification request identifier replaces the pending request" in normalized_notification_plan and
            "The isolated dateString mutation was rejected" in notification_identifier_verification and
            "xcodebuild was unavailable" in normalized_notification_verification,
            "unique notification identifier plan must record completed evidence",
            failures)
    require("Visit notifications use unique timestamp-UUID request identifiers" in " ".join(readme.split()) and
            "Visit notifications should use unique timestamp-UUID request identifiers" in " ".join(security.split()) and
            "Keep visit notification request identifiers unique with timestamp-UUID values" in " ".join(vision.split()) and
            "Made each local visit notification request identifier unique" in " ".join(changes.split()),
            "Project guidance must document unique visit notification request identifiers",
            failures)
